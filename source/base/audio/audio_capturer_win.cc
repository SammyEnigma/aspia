...
{
    LOG(INFO) << "Deinitialize audio capturer";
...
{
    LOG(INFO) << "Audio capturer initializing";
    ...
    if (FAILED(hr.Error()))
    {
        LOG(ERROR) << "Failed to create IMMDeviceEnumerator:" << hr;
        return false;
    }
    ...
    if (FAILED(hr.Error()))
    {
        LOG(ERROR) << "Failed to get IMMDevice:" << hr;
        return false;
    }
    ...
    if (FAILED(hr.Error()))
    {
        LOG(ERROR) << "Failed to get an IAudioClient:" << hr;
        return false;
    }
    ...
    if (FAILED(hr.Error()))
    {
        LOG(ERROR) << "IAudioClient::GetDevicePeriod failed:" << hr;
        return false;
    }
    ...
    if (FAILED(hr.Error()))
    {
        LOG(ERROR) << "Failed to get WAVEFORMATEX:" << hr;
        return false;
    }
    ...
    {
        LOG(ERROR) << "Host sampling rate is neither 44.1 kHz nor 48 kHz" << wave_format_ex_->nSamplesPerSec;
        return false;
    }
    ...
    LOG(INFO) << "Audio device period:" << audio_device_period_.count();
    ...
    if (FAILED(hr.Error()))
    {
        ...
        {
            LOG(ERROR) << "Audio capture may be blocked by your antivirus. Try disabling your "
    ...
    if (FAILED(hr.Error()))
    {
        LOG(ERROR) << "Failed to get an IAudioCaptureClient:" << hr;
        return false;
    }
    ...
    if (FAILED(hr.Error()))
    {
        LOG(ERROR) << "Failed to start IAudioClient:" << hr;
        return false;
    }
    ...
    LOG(INFO) << "Audio capturer initialized";
    return true;
}
...
void AudioCapturerWin::doCapture()
{
    ...
    if (FAILED(hr.Error()) && hr.Error() != last_capture_error_)
    {
        ...
        LOG(ERROR) << "Failed to capture an audio packet:" << hr;
    }
}
