    ...
    float initial_value, const float src[], int len, float smoothing_factor)
{
    ...
    // Compute z[n], z[n-1], z[n-2], and z[n-3] in parallel in lanes 3, 2, 1 and 0, respectively.
    ...
    for (i = 0; i < last_index; i += 4)
    {
        ...
        // Note: The compiler optimizes this to a single multiply-and-accumulate instruction:
    ...
    max_x4 = _mm_max_ps(max_x4, _mm_shuffle_ps(max_x4, max_x4, _MM_SHUFFLE(3, 3, 1, 1)));
